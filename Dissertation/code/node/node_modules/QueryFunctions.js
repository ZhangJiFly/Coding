...
module.exports = function(connection){
	...
	var getCourseListsInfo = function(callback){
		connection.query("Select Groups.GroupId,Credits, Groups.CourseListId, CourseId from Groups INNER JOIN GroupHasCourse on Groups.CourseListId = GroupHasCourse.CourseListId AND Groups.GroupId = GroupHasCourse.GroupId;", function(error, rows, file){
			if (error) throw error;
			var info = {};
			for (var i = 0; i<rows.length; i++){
				var courseListId = rows[i].CourseListId;
				var groupNum = rows[i].GroupId;
				var course = rows[i].CourseId
				if(info[courseListId] == undefined){
					info[courseListId] = {};
					info[courseListId][groupNum] = [course];
				}
				else if(info[courseListId][groupNum] == undefined){
					info[courseListId][groupNum] = [course]
				}
				else{
					info[courseListId][groupNum].push(course)
				}
			}
	    	callback(info);
		});
	}
	return {getSchoolCourses: getSchoolCourses, degreeCourseLists: degreeCourseLists, courseLists: courseLists, singleDegree: singleDegree, degrees: degrees, advisees: advisees, courses: courses, updateGroup: updateGroup, updateCourseList: updateCourseList, updateStart: updateStart, validUser:validUser, getCourseListsInfo:getCourseListsInfo};
}