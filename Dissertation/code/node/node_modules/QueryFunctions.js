	...
	var degreeCourseListsTotal = function(degree, callback){
		connection.query("SELECT Groups.GroupId,Groups.Minimum, DegreeHasCourseList.Level, Groups.Maximum, Groups.CourseListId, GroupHasCourse.CourseId, Course.Semester, Course.Name, Course.Level, Course.Credit, Course.Description, Course.Assessment, Course.MainAssessment, Course.EntryRequirements FROM DegreeHasCourseList, Course, Groups INNER JOIN GroupHasCourse ON Groups.CourseListId = GroupHasCourse.CourseListId AND Groups.GroupId = GroupHasCourse.GroupId WHERE DegreeHasCourseList.CourseListId = groups.CourseListId AND GroupHasCourse.CourseId = Course.CourseId AND Groups.CourseListId IN (SELECT CourseListId FROM DegreeHasCourseList WHERE Degree = ?) ORDER BY DegreeHasCourseList.Level;", degree, function(error, rows, file){
			...
			for (var i = 0; i<rows.length; i++){
				...
				var courseInfo = {CourseId: rows[i].CourseId, Name: rows[i].Name, Level: rows[i].Level, Credit: rows[i].Credit, Semester: rows[i].Semester, Description: rows[i].Description, Assessment: rows[i].Assessment, MainAssessment: rows[i].MainAssessment, Level:rows[i].Level}
