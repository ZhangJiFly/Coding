...
module.exports = function(connection){
	...
	var updateCourseList = function(matric, courseId){
	  var values = [];
	  values.push(data.listName);
	  values.push(data.school);
	  values.push(data.level);
	...
	var updateGroup = function(data){
		...
		for (var i=0; i<groups.length;i++){
	 	...
	 	var SQL = createSQL(groupValues, "INSERT INTO Groups (Credits, CourseListId, GroupId) VALUES ");
	...
	connection.query("SELECT Groups.GroupId,Credits, Groups.CourseListId, GroupHasCourse.CourseId, Course.Name, Course.Level, Course.Credit FROM Course, Groups INNER JOIN GroupHasCourse ON Groups.CourseListId = GroupHasCourse.CourseListId AND Groups.GroupId = GroupHasCourse.GroupId WHERE GroupHasCourse.CourseId = Course.CourseId AND Groups.CourseListId IN (SELECT CourseListId FROM DegreeHasCourseList WHERE Degree = ?);", degree, function(error, rows, file){
		if (error) throw error;
		var info = {};
	...
	var getCourseListsInfo = function(callback){
		connection.query("Select Groups.GroupId,Credits, Groups.CourseListId, GroupHasCourse.CourseId, Course.Name, Course.Level, Course.Credit from Course, Groups INNER JOIN GroupHasCourse on Groups.CourseListId = GroupHasCourse.CourseListId AND Groups.GroupId = GroupHasCourse.GroupId WHERE GroupHasCourse.CourseId = Course.CourseId", function(error, rows, file){
			if (error) throw error;
			var info = {};
			for (var i = 0; i<rows.length; i++){
				var courseListId = rows[i].CourseListId;
	...
	return {getStudentCourses:getStudentCourses, getAdvisor:getAdvisor, getSchoolCourses: getSchoolCourses, degreeCourseLists: degreeCourseLists, courseLists: courseLists, singleDegree: singleDegree, degrees: degrees, advisees: advisees, courses: courses, updateGroup: updateGroup, updateCourseList: updateCourseList, updateStart: updateStart, validUser:validUser, getCourseListsInfo:getCourseListsInfo, updateDegreeCourseLists:updateDegreeCourseLists, degreeCourseListsTotal:degreeCourseListsTotal};
}