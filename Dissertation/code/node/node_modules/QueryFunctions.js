...
module.exports = function(connection){
	...
	var updateCourseList = function(data, school){
	  ...
	  values.push(school);
	  values.push(data.listName);
	  connection.query("INSERT INTO Courselist (CourseListId, School) VALUES (?,?) ON DUPLICATE KEY UPDATE CourseListId=?;",values, function(error, insertId){
	    if (error) throw error;
	  });
	}

	var updateCoursesTaken = function(matric, courseId, callback){
	  connection.query("INSERT INTO StudentHasCourse (Matric, CourseId) VALUES (?,?) ON DUPLICATE KEY UPDATE CourseId=?;",[matric, courseId, courseId], function(error, insertId){
	    if (error) throw error;
	  });
	  callback(200);
	}
	...
	var createSQL = function(values, start){
		var SQL = start;
	  	...
	  	SQL = SQL.concat("ON DUPLICATE KEY UPDATE CourseListId=?;");
	  	return SQL ;
	}
	...
	var updateGroup = function(data){
		...
		for (var i=0; i<groups.length;i++){
			...
			for (var j = 0; j<groups[i].courses.length;j++){
			  groupCourseValues.push([i, data.listName, groups[i].courses[j].CourseId, groups[i].courses[j].CourseId])
			}
		}
	...
	connection.query("SELECT Groups.GroupId,Groups.Minimum, DegreeHasCourseList.Year, Groups.Maximum, Groups.CourseListId, GroupHasCourse.CourseId, Course.Semester, Course.Name, Course.Level, Course.Credit, Course.Description, Course.Assessment, Course.MainAssessment, Course.EntryRequirements FROM DegreeHasCourseList, Course, Groups INNER JOIN GroupHasCourse ON Groups.CourseListId = GroupHasCourse.CourseListId AND Groups.GroupId = GroupHasCourse.GroupId WHERE DegreeHasCourseList.CourseListId = groups.CourseListId AND GroupHasCourse.CourseId = Course.CourseId AND Groups.CourseListId IN (SELECT CourseListId FROM DegreeHasCourseList WHERE Degree = ?) ORDER BY DegreeHasCourseList.Year;", degree, function(error, rows, file){
		if (error) throw error;
		var info = {};
		for (var i = 0; i<rows.length; i++){
			var courseListId = rows[i].Year
			var groupNum = rows[i].GroupId;
			var min = rows[i].Minimum;
			var max = rows[i].Maximum;
			var courseInfo = {CourseId: rows[i].CourseId, Name: rows[i].Name, Level: rows[i].Level, Credit: rows[i].Credit, Semester: rows[i].Semester, Description: rows[i].Description, Assessment: rows[i].Assessment, MainAssessment: rows[i].MainAssessment, Level:rows[i].Year}
			if(info[courseListId] == undefined){
				info[courseListId] = {};
				info[courseListId][groupNum] = {}
				info[courseListId][groupNum].courses = [courseInfo];
				info[courseListId][groupNum].min = min
				info[courseListId][groupNum].max = max
			}
				...
				info[courseListId][groupNum].courses.push(courseInfo);
			}
		}
    	callback(info);
	});
	...
	return {updateCoursesTaken:updateCoursesTaken,getStudentCourses:getStudentCourses, getAdvisor:getAdvisor, getSchoolCourses: getSchoolCourses, degreeCourseLists: degreeCourseLists, courseLists: courseLists, singleDegree: singleDegree, degrees: degrees, advisees: advisees, studentTakesCourses:studentTakesCourses, updateGroup: updateGroup, updateCourseList: updateCourseList, updateStart: updateStart, validUser:validUser, getCourseListsInfo:getCourseListsInfo, updateDegreeCourseLists:updateDegreeCourseLists, degreeCourseListsTotal:degreeCourseListsTotal};
}